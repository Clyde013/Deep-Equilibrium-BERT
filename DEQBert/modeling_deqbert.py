...
# DEQ Override: DEQ layer is a single RobertaLayer with residual input injection
class DEQBertLayer(nn.Module):
    def __init__(self, config):
        super().__init__()

        # normal forward pass params
        ...
        # DEQ params
        # since the solvers only accept the hidden states, the tuple output of the layer
        # has to cache the attentions to be output after the final iteration
        self.cache_outputs = None
        self.hook = None
        if config.f_solver == "anderson":
            self.f_solver = anderson
        else:
            self.f_solver = broyden
        if config.b_solver == "anderson":
            self.b_solver = anderson
        else:
            self.b_solver = broyden
        self.f_thres = config.f_thres
    ...
    def forward(
        ...
        f = lambda x: self._forward(x.transpose(1, 2), input_injection, attention_mask, head_mask,
                                    encoder_hidden_states, encoder_attention_mask, past_key_value,
                                    output_attentions).transpose(1, 2)

        # initial estimate of fixed_point. hidden_states is [batch_size, seq_len, hidden_size].
        # z0 should be [batch_size, hidden_size, seq_len]
        ...
        # remember the transpose back the output (new_z_star) to [batch_size, seq_len, hidden_size]
...
class DEQBertForSequenceClassification(DEQBertPreTrainedModel):
    _keys_to_ignore_on_load_missing = [r"position_ids"]
...
class DEQBertClassificationHead(nn.Module):
...
class DEQBertForQuestionAnswering(DEQBertPreTrainedModel):
    _keys_to_ignore_on_load_unexpected = [r"pooler"]
    _keys_to_ignore_on_load_missing = [r"position_ids"]
